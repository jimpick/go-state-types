...
import (
	...
	"github.com/jimpick/go-state-types/builtin/v11/account"
	"github.com/jimpick/go-state-types/builtin/v11/cron"
	"github.com/jimpick/go-state-types/builtin/v11/datacap"
	"github.com/jimpick/go-state-types/builtin/v11/eam"
	"github.com/jimpick/go-state-types/builtin/v11/evm"
	init_ "github.com/jimpick/go-state-types/builtin/v11/init"
	"github.com/jimpick/go-state-types/builtin/v11/market"
	"github.com/jimpick/go-state-types/builtin/v11/miner"
	"github.com/jimpick/go-state-types/builtin/v11/multisig"
	"github.com/jimpick/go-state-types/builtin/v11/paych"
	"github.com/jimpick/go-state-types/builtin/v11/power"
	"github.com/jimpick/go-state-types/builtin/v11/reward"
	"github.com/jimpick/go-state-types/builtin/v11/system"
	"github.com/jimpick/go-state-types/builtin/v11/util/smoothing"
	"github.com/jimpick/go-state-types/builtin/v11/verifreg"
)

func main() {
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/system/cbor_gen.go", "system",
		// actor state
		system.State{},
	); err != nil {
		panic(err)
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/account/cbor_gen.go", "account",
		// actor state
		account.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/cron/cbor_gen.go", "cron",
		// actor state
		cron.State{},
		cron.Entry{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/reward/cbor_gen.go", "reward",
		// actor state
		reward.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/multisig/cbor_gen.go", "multisig",
		// actor state
		multisig.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/paych/cbor_gen.go", "paych",
		// actor state
		paych.State{},
		paych.LaneState{},
		//method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/power/cbor_gen.go", "power",
		// actors state
		power.State{},
		power.Claim{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/market/cbor_gen.go", "market",
		// actor state
		market.State{},
		market.DealState{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/miner/cbor_gen.go", "miner",
		// actor state
		miner.State{},
		miner.MinerInfo{},
		miner.Deadlines{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/verifreg/cbor_gen.go", "verifreg",
		// actor state
		verifreg.State{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/datacap/cbor_gen.go", "datacap",
		// actor state
		datacap.State{},
		datacap.TokenState{},
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/util/smoothing/cbor_gen.go", "smoothing",
		smoothing.FilterEstimate{},
	); err != nil {
		panic(err)
	}

	if err := gen.WriteTupleEncodersToFile("./builtin/v11/init/cbor_gen.go", "init",
		// actor state
		init_.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/evm/cbor_gen.go", "evm",
		// actor state
		evm.Tombstone{},
		evm.State{},
		// method params and returns
	...
	if err := gen.WriteTupleEncodersToFile("./builtin/v11/eam/cbor_gen.go", "eam",
		// method params and returns
		eam.CreateParams{},
		eam.CreateReturn{},
		eam.Create2Params{},
